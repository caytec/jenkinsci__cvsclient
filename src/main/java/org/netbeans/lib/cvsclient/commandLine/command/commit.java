...
public class commit extends AbstractCommandProvider {
    ...
    private static String getEditorProcess(String editor) {
        if (editor == null) {
            if (System.getProperty("os.name").startsWith("Windows")) {
                ...
                editor = null;// "vi"; - do not use 'vi'. It's not executed
    ...
    private static File createTempFile(final File[] args, final File tmpDir) throws IOException {
        File template = null;
    ...
    private static String createMessage(final File[] args, final GlobalOptions gopt) {
        File temp = null;
        ...
        try {
            ...
            final String editorProcess = getEditorProcess(gopt.getEditor());
            ...
            if (returnCode != 0) {
                ...
                String line;
                final StringBuffer message = new StringBuffer((int) temp.length());
    ...
    public Command createCommand(final String[] args, final int index, final GlobalOptions gopt, String workDir) {
        final CommitCommand command = new CommitCommand();
        ...
        if ((command.getMessage() == null) && (command.getLogMessageFromFile() == null)) {
            final String message = createMessage(fileArgs, gopt);
            if (message == null) {
                throw new IllegalArgumentException(java.util.ResourceBundle.getBundle(
                                commit.class.getPackage().getName() + ".Bundle")
