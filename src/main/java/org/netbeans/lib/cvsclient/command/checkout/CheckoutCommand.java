...
/**
 * The checkout command. This handles the sending of the requests and the
 * processing of the responses from the server.
 * 
 * @author Robert Greig
 */
public class CheckoutCommand extends BasicCommand implements TemporaryFileCreator {
     ...
     * 
     */
    private static final long serialVersionUID = 6280684709880423936L;
    ...
    /**
     * Handle modules that are already checked out. We check whether a module
     * has been checked out and if so we add it to the list of directories that
     * the superclass must send Modified requests for etc.
     */
    private void processExistingModules(final String localPath) {
        if (expandedModules.size() == 0) {
            return;
        }

        final List<File> list = new ArrayList<File>(expandedModules.size());
        for (final String string : expandedModules) {
            final String moduleName = string;
    ...
    public void execute(final ClientServices client, final EventManager em) throws CommandException,
        ...
        try {
            ...
            if (showModules || showModulesWithStatus) {
                // we need to initialize the builder first (is done in
                // BuildableCommand.execute()
     ...
     * building of requests. in checkout we operate with modules rather then
     * files. This produces problems in the following situation. If you have
     * something already checked out and want to checkout another module that is
     * not checked out yet, then there's nothing to be translated from modules
     * to files. and in such a case the localpathis assumed, which includes
     * non-relevant already checked out directories..
     */
    @Override
    ...
    private void postExpansionExecute(final ClientServices client, final EventManager em) throws CommandException,
        ...
        // Add a -- before the first file name just in case it looks like an
        // option.
    ...
    public String getCVSCommand() {
        final StringBuffer toReturn = new StringBuffer("checkout "); // NOI18N
    ...
    public void messageSent(final MessageEvent e) {
        ...
        if (pruneDirectories && (e.getMessage().indexOf(UPDATING) > 0)) {
            final String relPath = e.getMessage().substring(e.getMessage().indexOf(UPDATING) + UPDATING.length());
            final File file = new File(getLocalDirectory(), relPath);
