...
public final class PasswordsFile {
     ...
     * 
     * @param cvsRootString
     *            identifies repository session
     *            [:method:][[user][:password]@][hostname
     *            [:[port]]]/path/to/repository
     * @return scrambled password or <code>null</code>
     */
    public static String findPassword(final String cvsRootString) {
        final File passFile = new File(
                        System.getProperty("cvs.passfile", System.getProperty("user.home") + "/.cvspass"));
     ...
     */
    public static Collection<String> listRoots(final String prefix) {
        ...
        try {
            ...
            while ((line = reader.readLine()) != null) {
                line = normalize(line);
                final String elements[] = line.split(" "); // NOI18N
    ...
    /**
     * Writes scrambled password for given CVS root. Eliminates all previous
     * values and possible duplicities.
     * 
     * @param cvsRootString
     *            identifies repository session
     *            [:method:][[user][:password]@][hostname
     ...
     *             on write failure
     */
    public static void storePassword(final String cvsRootString, final String encodedPassword) throws IOException {
        final File passFile = new File(System.getProperty("cvs.passfile", System.getProperty("user.home")
    ...
    /**
     * Normalizes several possible line formats into 'normal' one that allows to
     * apply dumb string operations.
     */
